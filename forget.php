...
if ($result->num_rows > 0) { // Fix: use > 0 instead of > 1
    try {
        ...
        $mail->setFrom("$resetemail", 'EnVision Print');
        $mail->addAddress("$resetemail", '');

        $reset_link = "http://localhost/envisionprint/resetpassword.php?email=$resetemail";
        $message = '
        <html>
        <body>
            <p>Dear Maam/Sir,</p>
    ...
    ';
        ...
        $mail->isHTML(true);
        $mail->Subject = 'Important: Reset Your EnVision Print Account Password Now!';
        $mail->Body = $message;
